...
<body>
	...
	<div>
		<c:if
			test="${(login.user_id == vo.user_id and login.user_role_id == 0) or login.user_role_id == 2}">
			<input type="button" value="삭제하기"
				onClick="location.href='qna_del.do?qna_id=${vo.qna_id}'">
		</c:if>
	</div>
	<div>
		<div>제목</div>
	...
	<div>
		...
		<div>
			<c:if test="${login.user_role_id == 2}">
				...
				<input type="button" value="완료하기"
					onClick="location.href='qna_clear.do?qna_id=${vo.qna_id}'">
			</c:if>
		</div>
		<div>
			<c:forEach var="list" items="${ list }">
				<c:if
					test="${ list.qna_re_remove_lev eq 0 and login.user_role_id == 2}">
					<div>
						<input type="button" value="답글삭제"
							onClick="location.href='qna_reple_del.do?qna_re_ref=${ list.qna_re_ref }'">
					</div>
				</c:if>
				<div>
