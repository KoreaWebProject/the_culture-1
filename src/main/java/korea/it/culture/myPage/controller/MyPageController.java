...
;
	...
	// 나의 문의내역 화면으로 이동
	/*
	 * @RequestMapping("/myQna.do") public String moveMyQna(Model model) { int
	 * nowPage = 1; // 1로 첫페이지 번호를 가정
	 * 
	 * String user_id = request.getParameter("user_id"); String page =
	 * request.getParameter("page");// 기본자료형은 null값을 판단하지 못함
	 * 
	 ...
	 * 
	 * // 검색관련 내용 String search = request.getParameter("search");// 카테고리 String
	 * search_text = request.getParameter("search_text");// 검색어
	 * 
	 * // 검색어가 입력되어 있는 경우 if (search != null && !search.equalsIgnoreCase("all")) {
	 * switch (search) { case "name_subject_content": map.put("name", search_text);
	 * map.put("subject", search_text); map.put("content", search_text); break; case
	 * "name": map.put("name", search_text); break; case "subject":
	 * map.put("subject", search_text); break; case "content": map.put("content",
	 * search_text); break;
	 * 
	 ...
	 * 
	 * String search_param = String.format("search=%s&search_text=%s", search,
	 * search_text);
	 * 
	 * // 하단 페이지 메뉴 생성 String pageMenu =
	 * Paging.getPaging("myQna.do?user_id="+user_id, nowPage, // 현재페이지 row_id, // 전체
	 * 게시글 수 search_param, Common.Board.BLOCKLIST, // 한 페이지에 보여줄 게시글 수
	 * Common.Board.BLOCKPAGE); // 페이지 메뉴의 수
	 * 
	 * //바인딩 model.addAttribute("pageMenu", pageMenu); model.addAttribute("list",
	 * list);
	 * 
